<?php
require_once('staff_user/connect.php');
?>
<!DOCTYPE html>
<html>
<head>
...
<div class="container-fluid">
		...
		<div class="col-md-4 track_inp_box">
			<div class="">
				...
				<div class="form-group">
					Enter the conignment id provided to you<br />
				</div>
		...
		if(isset($_REQUEST['sub_req']))
		{
			...
			$consignment_id = $_REQUEST['id'];

			$query = "SELECT * FROM `product` WHERE `consignment__id` = '$consignment_id'";	
			...
			if(mysqli_num_rows($result) > 0)
			{
					...
					echo '
							...
							<div class="col-md-7">
								<p>'.$delivery_date.'<br />Not delivered</p>
							</div>
						</div>
					</div>
					';
					}
					elseif($status >= 1 && $attempts > 0 && $status != 3)
					{
					echo '
							<div class="tbo_head col-md-4">
				 ...
				 {	
					...
					$awb_query = "SELECT * FROM customer WHERE awb_number = '$consignment_id'";
					$awb_result = $db->query($awb_query);
					
					if(mysqli_num_rows($awb_result) > 0)
					{
					while($rows = mysqli_fetch_assoc($result))
					{
						$status = $rows['status'];
					...
					<div class="col-md-11 track_prod_info">
						...
						<div class="col-md-6 tpi_inf">
							...
							<div class="col-md-9">
								<table class="table table-striped ">
									<tr><td>Client Order Id</td><td>'.$client_order_id.'</td></tr>
									<tr><td>Price</td><td>'.$price.'</td></tr>
									<tr><td>Type</td><td>'.$dtype.'</td></tr>
								</table>
							</div>
				 ...
				 {
				 	echo '<div class="col-md-7 track_not_avail"><h2>Input awb number is not valid</h2></div>';
				 }
				 }
			}
